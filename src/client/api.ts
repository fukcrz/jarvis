...
export const api = {
  ...
  updateSettings: async (patch: Partial<Pick<AppSettings, "assistantName">>): Promise<AppSettings> => (await request<{ settings: AppSettings }>("/api/settings", { method: "PATCH", body: JSON.stringify(patch) })).settings,
  ...
  fetchProviderModels: async (providerId: string): Promise<FetchedModel[]> => (await request<{ models: FetchedModel[] }>(`/api/settings/providers/${encodeURIComponent(providerId)}/fetch-models`, { method: "POST", body: "{}" })).models,
