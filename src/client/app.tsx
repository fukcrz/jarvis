...
import { ArrowLeft, Bell, CircleAlert, FolderPlus, MoreVertical, Pencil, Plus, Puzzle, Sparkles, X } from "lucide-react";
import type { AppSettings, ComposerCommand, ImageAttachment, ModelDescriptor, SessionFileReference, SessionRef, SessionSummary, ThinkingLevel, Workspace, WorkspaceFile } from "../shared/protocol";
...
export function App() {
  ...
  const [uiMode, setUiMode] = useState<AppSettings["uiMode"]>("legacy");
  ...
  useEffect(() => {
    void Promise.all([loadWorkspaces(), api.settings().then((settings) => { setAssistantName(settings.assistantName); setUiMode(settings.uiMode); })]).catch((error: unknown) => setPageError(error instanceof Error ? error.message : "无法加载应用设置")).finally(() => setLoading(false));
  }, [loadWorkspaces]);
