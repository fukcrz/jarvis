...
const JARVIS_UI_NOTICE = [
  ...
  "Your reply is rendered as GitHub-flavored Markdown: headings, tables, task lists, links, fenced code with syntax highlighting, and ```mermaid blocks rendered as real diagrams. Prefer mermaid for flows, sequences, state machines, class and ER diagrams instead of ASCII art, and keep each diagram small enough to read (label the nodes, no decorative churn).",
  "Images: embed them with standard Markdown image syntax. ![](relative/path.png) resolves against the current workspace root; ![](/absolute/path.png) and ![](file:///absolute/path.png) resolve only when the path is inside a registered workspace. Video and audio use the same syntax and render as inline players (![](clip.mp4), ![](voice.mp3), plus http(s) and data: URLs), so never hand over a bare path or a download link for media either. Other local files linked as [name](relative/path.pdf) open in a preview or download when they are inside a registered workspace. Jarvis serves these through its /api/files endpoint and renders them inline, so the user sees them directly.",
